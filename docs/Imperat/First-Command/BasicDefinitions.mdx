...
## Raw-Argument/Input
A Raw-Argument, also known as `Raw-Input`, refers to the unprocessed input provided by the user when executing a command.
**It is the original string input that the user types**, which has not yet been parsed or converted into a specific data type.
Raw-Arguments are typically used as the initial input for command execution and are processed by the command system 
to extract meaningful information and convert it into the appropriate types for command execution.

## Argument
An Argument is a parameter that a `Command` can accept. 
It provides additional information or input required for the Command to execute properly. 
Arguments can be of various types, such as strings, numbers, or custom objects, and they can have specific validation rules.

Arguments are classified into four types based on their position and requirements:
- **Literal Argument**: A fixed value that must be provided for the Command to execute.Thus, It is used to define specific **subcommands**.
...
## Pathway
**A Pathway is a specific sequence of arguments and properties that define how a Command can be executed.**
**It represents a specific way to execute a Command**, including the required arguments, optional arguments, flags, 
and other metadata that define how the Command should be executed.
...
### Main Pathway
It's a pathway with **expected arguments**, meaning that it requires **specific sequence of input** from the user to execute the command.
A `Command` can have multiple main pathways, each with different arguments and properties,
allowing for various ways to execute the same command with different parameters.

example pathway: `/print <message>`.
In this pathway, the command is `print` and it has one required argument `message`.

### Default Pathway
**It is the unique pathway with NO arguments.**
Meaning that it can be executed without providing any arguments, 
making it the default way to execute the command when no arguments are supplied by the user.
It can be used to provide a default behavior for a Command.

example pathway: `/print`
In this pathway, the command is `print` and it can be executed without any arguments or with optional arguments like `message` in this case.
