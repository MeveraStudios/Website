...
Here we have a complex scenario where we want to parse two separate inputs (x, y and z) into a single `Point2D` object,
...
What defines a compound argument-type is the number of parameters/raw-argument it consumes,
thus, its crucial to override `ArgumentType#getNumberOfParametersToConsume` to define the argument of this type as a compound argument.
:::
