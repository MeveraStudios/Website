...
Learn more about creating/integrating with imperat's API in the [Command Builders](/docs/Imperat/Command-Builders) section.
:::
...
By default, Imperat stores registered commands in an in-memory map keyed by lowercase name and alias.
That works for the overwhelming majority of cases, but some embedders need something else — persistence across restarts, distributed lookups across nodes, or audit logging on every register/unregister call.
