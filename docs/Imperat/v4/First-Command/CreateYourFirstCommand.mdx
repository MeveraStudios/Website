...
In v4 Imperat, each platform builder pre-registers gating-aware `ContextArgumentProvider`s for the standard derived-source views — `Player` only when the executor isn't console, `ConsoleCommandSender` only when it is, etc. The framework injects these at the same slot as your canonical source `S` based on the parameter type. The legacy `SourceProvider` registry from v3 has been removed; everything goes through the unified `ContextArgumentProvider` path now.
