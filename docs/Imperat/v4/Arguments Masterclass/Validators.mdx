...
Pre-Parse Validators are already supported through the `parse` method of your custom argument type — implement your validation logic before parsing the input into the desired type.
If the validation fails, throw a `CommandException` with a custom error message.
```java
...
```java
public class DateArgumentType extends SimpleArgumentType<PLATFORMSOURCE, Date> {
...
Please refer to the [InstanceFactory](/docs/Imperat/InstanceFactory) section for more details on how to achieve this.
:::
