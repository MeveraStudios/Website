...
```
:::note
`[target]` is automatically declared as an **optional** argument, when we used kotlin-default
:::
...
> **Note**: Make sure that the code contained by the suspend function uses thread-safe algorithms and APIs only, commands may run asynchronously and be incompatible with some APIs such as the Bukkit API.
